...
config account use the fee observed from their own recent swaps.
...
and loaded-accounts budget in the header). Use it for routes that do not fit a v0 transaction even
with lookup tables (`ALT_ADDRESSES`); `/swap` rejects an oversized v0 build with a message.
