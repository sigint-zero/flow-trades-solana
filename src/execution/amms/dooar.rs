...
pub struct DooarExecutor;

// Dooar uses SPL Token Swap compatible instruction layout
const SWAP_DISC: u8 = 1;

impl AmmExecutor for DooarExecutor {
    fn build_swap_ix(
        &self,
        order: &SwapOrder,
        ...
        let (pool, authority, token_a_vault, token_b_vault, pool_mint, fee_account, token_a_mint, _token_b_mint) =
            match pool_state {
                ...
                _ => return Err(TradeError::Execution("expected Dooar pool state".into())),
            };
